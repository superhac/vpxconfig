#!/usr/bin/env python3
"""Check that a release tag matches the version in vpinconfig/__init__.py.

    tools/check_version.py v0.5        exits 0 when the tag is "v" + the version, 1 otherwise

The version is set in one place, vpinconfig/__init__.py. A release is a git tag "v<version>" (for example v0.5),
...
def main(argv):
    ...
    if not re.fullmatch(r"\d+\.\d+(\.\d+)?", version):
        sys.exit(f"the version {version!r} in vpinconfig/__init__.py must look like 0.5 or 0.5.1")
    tag = argv[1]
    if tag != f"v{version}":
        sys.exit(f"the tag {tag} does not match the version in vpinconfig/__init__.py (expected v{version}). "
